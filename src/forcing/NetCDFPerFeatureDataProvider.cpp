    ...
    , sim_end_date_time_epoch(sim_end)
{
    ...
    std::vector<std::string> var_set = nc_manager->list_variables();

    // populate the ncvar and units caches...
    for (const auto& var_name : var_set)
    {
        auto ncvar = nc_manager->get_ncvar_by_name(var_name);
        ...
        try
        {
            native_units = ncvar->get_string_attribute("units");
        }
        catch(...)
        {
            native_units = "";
    ...
    auto ids = nc_manager->get_ncvar_by_name("ids");
    const auto& ids_dims = ids->get_dims(); 
    ...
    if ( ids_dims.size() != 1)
    {
        std::string msg = "Provided NetCDF file has an \"ids\" variable that is not a 1D array of catchments";
        LOG(LogLevel::FATAL, msg);
        throw std::runtime_error(msg);
    }

    auto num_ids = ids_dims[0];
    if (num_ids == 0)
    {
        std::string msg = "Provided NetCDF file has no catchments listed for variable  \"ids\"";
        LOG(LogLevel::FATAL, msg);
        throw std::runtime_error(msg);
    }
    ...
    // initalize the map of catchment-name to offset location
    size_t loc = 0;
    for(const auto& id : ids->get_string_array_values())
    {
    ...
    auto time_var = nc_manager->get_ncvar_by_name("Time");

    // Get the size of the time dimension
    size_t num_times = time_var->get_dims().size();

    std::vector<double> raw_time;

    try {
        auto dim_count = time_var->get_dim_count();
        // Old-format files have dimensions (catchment, time), new-format
        // files generated by the forcings engine have just (time)
        if (dim_count == 2) {
            if (time_var->get_dim_names()[0] != "catchment-id" || time_var->get_dim_names()[1] != "time") {
        ...
        raw_time = time_var->get_time_values();
    ...
    try {
        time_units = time_var->get_string_attribute("units");
...
{
    if (nc_manager != nullptr) {
        nc_manager->close_file();
    }
    nc_manager = nullptr;
}
...
double NetCDFPerFeatureDataProvider::get_value(const CatchmentAggrDataSelector& selector, ReSampleMethod m) 
{
    ...
    std::vector<std::string> dims = ncvar->get_dim_names();
    ...
    if (dims[0] == "time" && dims[1] == "catchment-id") {
        // Forcings Engine NetCDF output case
        dim_time = 0;
        dim_catchment = 1;
    } else if (dims[1] == "time" && dims[0] == "catchment-id") {
    ...
    size_t time_dim_size = ncvar->get_dim_size(dim_time);
    size_t catchment_dim_size = ncvar->get_dim_size(dim_catchment);
    ...
    for( size_t i = 0; i < cache_slices_t_n; i++ ) {
        ...
        std::string key = ncvar->get_name() + "|" + std::to_string(cache_t_idx) + "|" + std::to_string(slice_size);
        ...
        if(value_cache.contains(key)){
            ...
            try {
                ncvar->read_slice(start,count,cached->data());
...
std::shared_ptr<NetCDFVar> NetCDFPerFeatureDataProvider::get_ncvar(const std::string& name) const{
