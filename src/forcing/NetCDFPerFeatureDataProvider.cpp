    ...
    , sim_end_date_time_epoch(sim_end)
{
    ...
    try {
        ...
        if (dim_count == 2) {
            if (time_var.getDim(0).getName() != "catchment-id" || time_var.getDim(1).getName() != "time") {
                std::string message = "In NetCDF file '" + input_path + "',  'Time' variable dimensions don't match expectations";
    ...
    if (time_base_unit == "minutes") {
        ...
        std::string message = "In NetCDF file '" + input_path + "', time unit '" + time_base_unit + "' could not be converted";
        std::string throw_msg; throw_msg.assign(message);
        LOG(throw_msg, LogLevel::WARNING);
        throw std::runtime_error(throw_msg);
    }
...
double NetCDFPerFeatureDataProvider::get_value(const CatchmentAggrDataSelector& selector, ReSampleMethod m) 
{
    ...
    if (dims.size() != 2) {
        std::string message = "Variable dimension count isn't 2";
        std::string throw_msg; throw_msg.assign(message);
        LOG(throw_msg, LogLevel::WARNING);
        throw std::runtime_error(throw_msg);
    }
    if (dims[0].getName() == "time" && dims[1].getName() == "catchment-id") {
        ...
        std::string message = "Variable dimensions aren't 'time' and 'catchment-id'";
        std::string throw_msg; throw_msg.assign(message);
        LOG(throw_msg, LogLevel::WARNING);
        throw std::runtime_error(throw_msg);
    }
