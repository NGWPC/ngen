...
std::string Bmi_Fortran_Adapter::GetComponentName() {
    ...
    if (get_component_name(&bmi_model->handle, component_name) != BMI_SUCCESS) {
        LOG(LogLevel::FATAL, "failed to get model component name.");
        throw std::runtime_error("failed to get model component name.");
    }
...
double Bmi_Fortran_Adapter::GetCurrentTime() {
    ...
    if (get_current_time(&bmi_model->handle, &current_time) != BMI_SUCCESS) {
        LOG(LogLevel::FATAL, "failed to get current model time.");
        throw std::runtime_error("failed to get current model time.");
    }
    return current_time;
}
...
double Bmi_Fortran_Adapter::GetEndTime() {
    ...
    if (get_end_time(&bmi_model->handle, &end_time) != BMI_SUCCESS) {
        LOG(LogLevel::FATAL, "failed to get model end time.");
        throw std::runtime_error("failed to get model end time.");
    }
    return end_time;
}
...
double Bmi_Fortran_Adapter::GetStartTime() {
    ...
    if (get_start_time(&bmi_model->handle, &start_time) != BMI_SUCCESS) {
        LOG(LogLevel::FATAL, "failed to get model start time.");
        throw std::runtime_error("failed to get model start time.");
    }
    return start_time;
}
...
double Bmi_Fortran_Adapter::GetTimeStep() {
    ...
    if (get_time_step(&bmi_model->handle, &ts) != BMI_SUCCESS) {
        LOG(LogLevel::FATAL, "failed to get model time step size.");
        throw std::runtime_error("failed to get model time step size.");
    }
    return ts;
}
...
std::string Bmi_Fortran_Adapter::GetTimeUnits() {
    ...
    if (get_time_units(&bmi_model->handle, time_units_cstr) != BMI_SUCCESS) {
        LOG(LogLevel::FATAL, "failed to read time units from model.");
        throw std::runtime_error("failed to read time units from model.");
    }
...
void Bmi_Fortran_Adapter::GetValueAtIndices(std::string name, void *dest, int *inds, int count) {
     ...
     */
    LOG(LogLevel::FATAL, "Fortran BMI module integration does not currently support getting values by index");
...
int Bmi_Fortran_Adapter::GetVarItemsize(std::string name) {
    ...
    if (get_var_itemsize(&bmi_model->handle, name.c_str(), &size) != BMI_SUCCESS) {
        LOG(LogLevel::FATAL, "failed to get variable item size for " + name + ".");
        throw std::runtime_error("failed to get variable item size for " + name + ".");
    }
    return size;
}
...
int Bmi_Fortran_Adapter::GetVarNbytes(std::string name) {
    ...
    if (get_var_nbytes(&bmi_model->handle, name.c_str(), &size) != BMI_SUCCESS) {
        LOG(LogLevel::FATAL, "failed to get variable array size (i.e., nbytes) for " + name + ".");
        throw std::runtime_error("failed to get variable array size (i.e., nbytes) for " + name + ".");
    }
    return size;
}
...
std::string Bmi_Fortran_Adapter::GetVarUnits(std::string name) {
    ...
    if (get_var_units(&bmi_model->handle, name.c_str(), units_c_str) != BMI_SUCCESS) {
        LOG(LogLevel::FATAL, "failed to get variable units for " + name + ".");
        throw std::runtime_error("failed to get variable units for " + name + ".");
    }
...
std::string Bmi_Fortran_Adapter::GetVarLocation(std::string name) {
    ...
    if (get_var_location(&bmi_model->handle, name.c_str(), location_c_str) != BMI_SUCCESS) {
        LOG(LogLevel::FATAL, "failed to get variable location for " + name + ".");
        throw std::runtime_error("failed to get variable location for " + name + ".");
    }
...
int Bmi_Fortran_Adapter::GetVarGrid(std::string name) {
    ...
    if (get_var_grid(&bmi_model->handle, name.c_str(), &grid) != BMI_SUCCESS) {
        LOG(LogLevel::FATAL, "failed to get variable grid for " + name + ".");
        throw std::runtime_error("failed to get variable grid for " + name + ".");
    }
    return grid;
}
...
std::string Bmi_Fortran_Adapter::GetGridType(int grid_id) {
    ...
    if (get_grid_type(&bmi_model->handle, &grid_id, gridtype_c_str) != BMI_SUCCESS) {
        LOG(LogLevel::FATAL, "failed to get grid type for grid ID " + std::to_string(grid_id) + ".");
        throw std::runtime_error("failed to get grid type for grid ID " + std::to_string(grid_id) + ".");
    }
...
int Bmi_Fortran_Adapter::GetGridRank(int grid_id) {
    ...
    if (get_grid_rank(&bmi_model->handle, &grid_id, &gridrank) != BMI_SUCCESS) {
        LOG(LogLevel::FATAL, "failed to get grid rank for grid ID " + std::to_string(grid_id) + ".");
        throw std::runtime_error("failed to get grid rank for grid ID " + std::to_string(grid_id) + ".");
    }
    return gridrank;
}
...
int Bmi_Fortran_Adapter::GetGridSize(int grid_id) {
    ...
    if (get_grid_size(&bmi_model->handle, &grid_id, &gridsize) != BMI_SUCCESS) {
        LOG(LogLevel::FATAL, "failed to get grid size for grid ID " + std::to_string(grid_id) + ".");
        throw std::runtime_error("failed to get grid size for grid ID " + std::to_string(grid_id) + ".");
    }
    return gridsize;
}
...
void Bmi_Fortran_Adapter::SetValueAtIndices(std::string name, int *inds, int count, void *src) {
    ...
    */
    LOG(LogLevel::FATAL, "Fortran BMI module integration does not currently support setting values by index");
...
void Bmi_Fortran_Adapter::GetGridShape(int grid, int *shape) {
    if (get_grid_shape(&bmi_model->handle, &grid, shape) != BMI_SUCCESS) {
        LOG(LogLevel::FATAL, "failed to get grid " + std::to_string(grid) + " shape.");
        throw std::runtime_error("failed to get grid " + std::to_string(grid) + " shape.");
    }
}
...
void Bmi_Fortran_Adapter::GetGridSpacing(int grid, double *spacing) {
    if (get_grid_spacing(&bmi_model->handle, &grid, spacing) != BMI_SUCCESS) {
        LOG(LogLevel::FATAL, "failed to get grid " + std::to_string(grid) + " spacing.");
        throw std::runtime_error("failed to get grid " + std::to_string(grid) + " spacing.");
    }
}

void Bmi_Fortran_Adapter::GetGridOrigin(int grid, double *origin) {
    if (get_grid_origin(&bmi_model->handle, &grid, origin) != BMI_SUCCESS) {
        LOG(LogLevel::FATAL, "failed to get grid " + std::to_string(grid) + " origin.");
        throw std::runtime_error("failed to get grid " + std::to_string(grid) + " origin.");
    }
}

void Bmi_Fortran_Adapter::GetGridX(int grid, double *x) {
    if (get_grid_x(&bmi_model->handle, &grid, x) != BMI_SUCCESS) {
        LOG(LogLevel::FATAL, "failed to get grid " + std::to_string(grid) + " x.");
        throw std::runtime_error("failed to get grid " + std::to_string(grid) + " x.");
    }
}

void Bmi_Fortran_Adapter::GetGridY(int grid, double *y) {
    if (get_grid_y(&bmi_model->handle, &grid, y) != BMI_SUCCESS) {
        LOG(LogLevel::FATAL, "failed to get grid " + std::to_string(grid) + " y.");
        throw std::runtime_error("failed to get grid " + std::to_string(grid) + " y.");
    }
}

void Bmi_Fortran_Adapter::GetGridZ(int grid, double *z) {
    if (get_grid_z(&bmi_model->handle, &grid, z) != BMI_SUCCESS) {
        LOG(LogLevel::FATAL, "failed to get grid " + std::to_string(grid) + " z.");
        throw std::runtime_error("failed to get grid " + std::to_string(grid) + " z.");
    }
}
...
int Bmi_Fortran_Adapter::GetGridNodeCount(int grid) {
    ...
    if (get_grid_node_count(&bmi_model->handle, &grid, &count) != BMI_SUCCESS) {
        LOG(LogLevel::FATAL, "failed to get grid " + std::to_string(grid) + " node count.");
        throw std::runtime_error("failed to get grid " + std::to_string(grid) + " node count.");
    }
    return count;
}
...
int Bmi_Fortran_Adapter::GetGridEdgeCount(int grid) {
    ...
    if (get_grid_edge_count(&bmi_model->handle, &grid, &count) != BMI_SUCCESS) {
        LOG(LogLevel::FATAL, "failed to get grid " + std::to_string(grid) + " edge count.");
        throw std::runtime_error("failed to get grid " + std::to_string(grid) + " edge count.");
    }
    return count;
}
...
int Bmi_Fortran_Adapter::GetGridFaceCount(int grid) {
    ...
    if (get_grid_face_count(&bmi_model->handle, &grid, &count) != BMI_SUCCESS) {
        LOG(LogLevel::FATAL, "failed to get grid " + std::to_string(grid) + " face count.");
        throw std::runtime_error("failed to get grid " + std::to_string(grid) + " face count.");
    }
    return count;
}
...
void Bmi_Fortran_Adapter::GetGridEdgeNodes(int grid, int *edge_nodes) {
    if (get_grid_edge_nodes(&bmi_model->handle, &grid, edge_nodes) != BMI_SUCCESS) {
        LOG(LogLevel::FATAL, "failed to get grid " + std::to_string(grid) + " edge nodes.");
        throw std::runtime_error("failed to get grid " + std::to_string(grid) + " edge nodes.");
    }
}
...
void Bmi_Fortran_Adapter::GetGridFaceEdges(int grid, int *face_edges) {
    if (get_grid_face_edges(&bmi_model->handle, &grid, face_edges) != BMI_SUCCESS) {
        LOG(LogLevel::FATAL, "failed to get grid " + std::to_string(grid) + " face edges.");
        throw std::runtime_error("failed to get grid " + std::to_string(grid) + " face edges.");
    }
}
...
void Bmi_Fortran_Adapter::GetGridFaceNodes(int grid, int *face_nodes) {
    if (get_grid_face_nodes(&bmi_model->handle, &grid, face_nodes) != BMI_SUCCESS) {
        LOG(LogLevel::FATAL, "failed to get grid " + std::to_string(grid) + " face nodes.");
        throw std::runtime_error("failed to get grid " + std::to_string(grid) + " face nodes.");
    }
}
...
void Bmi_Fortran_Adapter::GetGridNodesPerFace(int grid, int *nodes_per_face) {
    if (get_grid_nodes_per_face(&bmi_model->handle, &grid, nodes_per_face) != BMI_SUCCESS) {
        LOG(LogLevel::FATAL, "failed to get grid " + std::to_string(grid) + " nodes per face.");
        throw std::runtime_error("failed to get grid " + std::to_string(grid) + " nodes per face.");
    }
}
